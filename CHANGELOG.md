...
- Makes the runtime dependency lock cross-platform and time-stable (`uv pip compile --universal --exclude-newer`) with a `scripts/regenerate_dependency_lock.sh` helper, so the publication gate reproduces from a clean clone on any platform.
